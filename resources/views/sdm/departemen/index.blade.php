...
<div class="grid md:grid-cols-2 gap-4">
    ...
    <a href="{{ route('sdm.departemen.show', $d->id) }}" class="card p-5 hover:border-accent transition-colors block">
        <div class="flex items-start justify-between gap-3 mb-4">
            ...
            @php $pk = $d->pemberiKeputusanUtama(); @endphp
            @if ($pk)
            <span class="badge badge-disetujui shrink-0">{{ $pk->name }}</span>
            @else
            <span class="badge badge-ditolak shrink-0">Belum Ada Pemberi Keputusan</span>
            @endif
        </div>
